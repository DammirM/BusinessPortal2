...
namespace BusinessPortal2.Controllers
{
    ...
    public class LeaveRequestAdminController : ControllerBase
    {
        ...
        public async Task<IActionResult> ExportData(DateTime start, DateTime end)
        {
            ...
            Console.WriteLine( $"Request End Date: {request.EndDate}");
                return request.EndDate >= start && request.StartDate <= end;
            }).ToList();

            if (leaveRequestsInTimeframe.Any())
            {
                leaves.AddRange(leaveRequestsInTimeframe);
                response.isSuccess = true;
                response.StatusCode = System.Net.HttpStatusCode.OK;
                response.Result = leaves;
                return Ok(response);
            }
            else
            {
                response.Errors.Add("No Approved request with that timeframe found!");
                return BadRequest(response);
            }
